...
const Status extractSequenceIdInternal(const tensorflow::TensorProto& proto, uint64_t& sequenceId) {
...
static const Status extractSequenceControlInputInternal(const tensorflow::TensorProto& proto, uint32_t& sequenceControlInput) {
...
template <>
const Status extractSpecialKeys(const tensorflow::serving::PredictRequest* request, SequenceProcessingSpec& sequenceProcessingSpec) {
    uint64_t sequenceId = 0;
    uint32_t sequenceControlInput = 0;
    Status status;
    auto it = request->inputs().find("sequence_id");
    if (it != request->inputs().end()) {
        status = extractSequenceIdInternal(it->second, sequenceId);
        if (!status.ok())
            return status;
    }
    it = request->inputs().find("sequence_control_input");
    if (it != request->inputs().end()) {
        status = extractSequenceControlInputInternal(it->second, sequenceControlInput);
        if (!status.ok())
            return status;
    }
...
// TODO depends only on stateful & TFS
static void writeSequenceId2TFSResponse(SequenceProcessingSpec& sequenceProcessingSpec, tensorflow::serving::PredictResponse* response) {
...
Status StatefulRequestProcessor<tensorflow::serving::PredictRequest, tensorflow::serving::PredictResponse>::postInferenceProcessing(tensorflow::serving::PredictResponse* response, ov::InferRequest& inferRequest) {
    ...
    writeSequenceId2TFSResponse(sequenceProcessingSpec, response);
    return StatusCode::OK;
}
template <>
